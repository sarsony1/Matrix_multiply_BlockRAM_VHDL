...
void transposeMatrix(unsigned char arr[16][16],int size ,unsigned char arr2[16][16])
{
    int i,j;
    for(i=0;i<size;i++)
    {
...
int main(int argc, const char *argv[]) {
    uint8 a[16][16] ,I[16][16] , b[16][16] , c[16][16] ,p ,q ,start =1;
    int num =21;
	...
	if ( isCommCapable ) {
        ...
        transposeMatrix(I,16,b) ;
        
        status = flWriteChannel(handle, 1000, 0x00, 1, &start, &error);
        CHECK(num++);
        ...
        for (p=0; p<16; p++)
        {
            for (q=0; q<16; q++)
            {
                status = flWriteChannel(handle, 1000, 0x03, 1, &a[p][q], &error);
                CHECK(num++);
                status = flWriteChannel(handle, 1000, 0x01, 1, &p, &error);
                CHECK(num++);
                status = flWriteChannel(handle, 1000, 0x02, 1, &q, &error);
                CHECK(num++);
        ...
        for (p=0; p<16; p++)
        {
            status = flWriteChannel(handle, 1000, 0x04, 1, &p, &error);
            CHECK(num++);
            for (q=0; q<16; q++)
            {
                status = flWriteChannel(handle, 1000, 0x03, 1, &b[p][q], &error);
                CHECK(num++);
                status = flWriteChannel(handle, 1000, 0x01, 1, &p, &error);
                CHECK(num++);
                status = flWriteChannel(handle, 1000, 0x02, 1, &q, &error);
        ...
        for (p=0; p<16; p++)
        {
            for (q=0; q<16; q++)
            {
                status = flReadChannel(handle, 1000, 0x04, 1, &c[p][q], &error);
                CHECK(num++);
                status = flWriteChannel(handle, 1000, 0x01, 1, &p, &error);
                CHECK(num++);
                status = flWriteChannel(handle, 1000, 0x02, 1, &q, &error);
